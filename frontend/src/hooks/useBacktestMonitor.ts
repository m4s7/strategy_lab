...
import { useWebSocketSubscription, useWebSocketMultiSubscription } from '@/lib/websocket/hooks';
...
export function useBacktestMonitor() {
  ...
  const { data: monitorData } = useWebSocketMultiSubscription<BacktestMonitorData>(
    ...
    if (savedState) {
      try {
        ...
        Object.entries(parsed).forEach(([id, monitor]) => {
          ...
          if (monitorData.status === 'running' || monitorData.status === 'paused') {
  ...
  const addMonitor = useCallback((backtestId: string, initialData?: BacktestMonitorData) => {
    if (initialData) {
  ...
  const updateMonitor = useCallback((backtestId: string, updates: Partial<BacktestMonitorData>) => {
    setMonitors(prev => ({
      ...prev,
      [backtestId]: {
        ...prev[backtestId],
  ...
  const getMonitorsByStatus = useCallback((status: BacktestMonitorData['status']) => {
  ...
  const activeMonitors = useCallback(() => {
    return Object.entries(monitors)
      .filter(([, monitor]) => monitor.status === 'running' || monitor.status === 'paused')
      .map(([id, monitor]) => ({ id, ...monitor }));
  }, [monitors]);
  ...
  const clearCompleted = useCallback(() => {
    setMonitors(prev => {
      ...
      Object.entries(prev).forEach(([id, monitor]) => {
        if (monitor.status !== 'completed' && monitor.status !== 'aborted' && monitor.status !== 'failed') {
          updated[id] = monitor;
        }
      });
      return updated;
    ...
    setSubscriptions(prev => 
      prev.filter(id => {
        ...
        return monitor && monitor.status !== 'completed' && monitor.status !== 'aborted' && monitor.status !== 'failed';
      })
    );
  }, [monitors]);

  // Control functions that would interact with backend
  const abortBacktest = useCallback(async (backtestId: string) => {
    try {
  ...
  const pauseBacktest = useCallback(async (backtestId: string) => {
    try {
  ...
  const resumeBacktest = useCallback(async (backtestId: string) => {
    try {
  ...
  return {
    monitors: Object.entries(monitors).map(([id, monitor]) => ({ id, ...monitor })),
