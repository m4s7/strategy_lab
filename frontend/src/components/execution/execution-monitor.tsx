...
export function ExecutionMonitor({ 
  ...
  const canPause = ['processing', 'loading_data', 'calculating_metrics'].includes(execution.status);
  const canResume = execution.status === 'paused';
  const canCancel = !['completed', 'failed', 'cancelled'].includes(execution.status);
  const canDelete = ['completed', 'failed', 'cancelled'].includes(execution.status);
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {execution.progress.current_tick > 0 && (
            <div className="text-xs text-muted-foreground">
              Processed {formatNumber(execution.progress.current_tick)} of {formatNumber(execution.progress.total_ticks)} ticks
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="text-center p-2 bg-muted/50 rounded">
            <div className="text-lg font-medium">{execution.metrics.total_trades}</div>
          ...
          <div className="text-center p-2 bg-muted/50 rounded">
            <div className={`text-lg font-medium ${
              execution.metrics.total_pnl >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
        ...
        <div className="grid grid-cols-3 gap-3 text-sm">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span>Runtime: {formatDuration(execution.start_time, execution.end_time)}</span>
          </div>
        </div>
        ...
        {execution.estimated_end_time && execution.status === 'processing' && (
          <div className="text-sm text-muted-foreground">
            ...
            Estimated completion: {formatDistanceToNow(new Date(execution.estimated_end_time), { addSuffix: true })}
          </div>
        )}
      </CardContent>
    </Card>
