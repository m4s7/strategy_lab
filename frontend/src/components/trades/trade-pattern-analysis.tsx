...
export function TradePatternAnalysis({
  trades,
  className,
}: TradePatternAnalysisProps) {
  const [groupBy, setGroupBy] = useState<GroupingCriteria>("hourOfDay");
  const [metric, setMetric] = useState<
    "count" | "pnl" | "winRate" | "avgReturn"
  >("pnl");
      ...
      <CardHeader>
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center space-x-2">
            ...
            <Select
              value={groupBy}
              onValueChange={(value: GroupingCriteria) => setGroupBy(value)}
            >
  ...
  const insights = useMemo(() => {
      ...
      insights.push({
        ...
        message: `Best ${groupBy}: ${best.group} with ${
          metric === "count"
            ? `${best.value} trades`
            : metric === "pnl"
            ? `$${best.value.toFixed(0)} total P&L`
            : metric === "winRate"
            ? `${(best.value * 100).toFixed(1)}% win rate`
      ...
      insights.push({
        ...
        message: `Worst ${groupBy}: ${worst.group} with ${
          metric === "count"
            ? `${worst.value} trades`
            : metric === "pnl"
            ? `$${worst.value.toFixed(0)} total P&L`
            : metric === "winRate"
            ? `${(worst.value * 100).toFixed(1)}% win rate`
    ...
    if (groupBy === "hourOfDay") {
      ...
      if (marketHours.length > 0 && afterHours.length > 0) {
        const marketTotal = marketHours.reduce(
          (sum, d) => sum + d.details.totalPnl,
          0
        );
        const afterTotal = afterHours.reduce(
          (sum, d) => sum + d.details.totalPnl,
          0
        );
        ...
        insights.push({
          type: "info",
          icon: Clock,
          message: `Market hours: $${marketTotal.toFixed(
            0
          )} vs After hours: $${afterTotal.toFixed(0)}`,
        });
      }
    }
      ...
      if (longData && shortData) {
        ...
        if (Math.abs(longWinRate - shortWinRate) > 0.1) {
          insights.push({
            ...
            icon: Target,
            message: `${
              longWinRate > shortWinRate ? "Long" : "Short"
            } trades perform better: ${(
              (longWinRate > shortWinRate ? longWinRate : shortWinRate) * 100
            ).toFixed(1)}% win rate vs ${(
              (longWinRate > shortWinRate ? shortWinRate : longWinRate) * 100
            ).toFixed(1)}%`,
          });
        }
      }
                ...
                <div
                  className={`p-2 rounded-lg ${
                    insight.type === "success"
                      ...
                      ? "bg-orange-100 text-orange-600 dark:bg-orange-950/20"
