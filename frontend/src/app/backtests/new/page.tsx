...
export default function BacktestExecutionPage() {
  ...
  const handleStartBacktest = async () => {
    ...
    try {
      ...
      toast.success(`Backtest started! Execution ID: ${result.execution_id.slice(0, 8)}...`);
      
      // Clear session storage after successful start
      sessionStorage.removeItem('backtestConfig');
      sessionStorage.removeItem('dataConfig');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to start backtest');
    } finally {
      setStartingExecution(false);
    }
  };

  const handleControlExecution = async (executionId: string, action: string) => {
    try {
      await controlExecution(executionId, action);
      toast.success(`Execution ${action}ed successfully`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : `Failed to ${action} execution`);
    }
  };
  ...
  const handleDeleteExecution = async (executionId: string) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to delete execution');
    }
  };
            ...
            {strategyConfig ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{strategyConfig.strategy_name}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="pt-2 border-t">
                  <details className="text-sm">
                    <summary className="cursor-pointer text-muted-foreground">View Parameters</summary>
                    <div className="mt-2 space-y-1">
                      {Object.entries(strategyConfig.parameters).map(([key, value]) => (
      ...
      {strategyConfig && dataConfig && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your backtest configuration is ready. Click start to begin execution.
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <div className="grid gap-4">
            {activeExecutions.map(execution => (
              <ExecutionMonitor
                key={execution.id}
                execution={execution}
                onControl={(action) => handleControlExecution(execution.id, action)}
