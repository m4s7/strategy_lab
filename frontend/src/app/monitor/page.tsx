...
export default function MonitorPage() {
  ...
  const runningExecutions = executions.filter(e => e.status === 'running' || e.status === 'paused');
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getSystemHealthColor(systemStatus?.cpu_usage || 0)}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getSystemHealthColor(systemStatus?.memory_usage || 0)}`}>
      ...
      <Card>
        ...
        <CardContent>
          ...
          ) : runningExecutions.length === 0 && queuedExecutions.length === 0 ? (
            <div className="text-center py-8">
              <BarChart3 className="mx-auto h-12 w-12 text-muted-foreground" />
              <h3 className="mt-4 text-lg font-medium">No active executions</h3>
              <p className="text-muted-foreground">Start a backtest to see real-time monitoring</p>
            </div>
          ) : (
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-medium">{execution.strategy_name}</h4>
                          <p className="text-sm text-muted-foreground">
                            Started {format(new Date(execution.started_at), 'HH:mm:ss')}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                      <div>
                        ...
                        <p className="text-lg font-semibold">
                          {formatCurrency(execution.current_equity || execution.initial_capital)}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Unrealized P&L</p>
                        <p className={`text-lg font-semibold ${
                          (execution.unrealized_pnl || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                        }`}>
                      ...
                      <div>
                        ...
                        <p className="text-lg font-semibold">{execution.trades_completed || 0}</p>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-medium">{execution.strategy_name}</h4>
                          <p className="text-sm text-muted-foreground">
                            Queued at {format(new Date(execution.created_at), 'HH:mm:ss')}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{format(currentTime, 'HH:mm:ss')}</p>
              </div>
            </div>
            <div className="flex items-start space-x-3">
            ...
            <div className="flex items-start space-x-3">
              <CheckCircle className="h-5 w-5 text-green-600 mt-0.5" />
              <div>
                <p className="text-sm font-medium">Data feed connection restored</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Executions Completed:</span>
