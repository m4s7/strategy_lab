...
async def seed_database():
    ...
    async with async_session_factory() as session:
        try:
            ...
            statuses = [BacktestStatus.COMPLETED, BacktestStatus.RUNNING, BacktestStatus.PENDING, BacktestStatus.FAILED]
            ...
            for i, strategy in enumerate(sample_strategies):
                ...
                if status in [BacktestStatus.COMPLETED, BacktestStatus.FAILED]:
                    ...
                    await BacktestOperations.update_backtest_status(
                        ...
                        backtest.id,
                        status,
                        error_message=f"Sample error for {strategy}" if status == BacktestStatus.FAILED else None
                    )
            ...
            for backtest, status in created_backtests:
                if status == BacktestStatus.COMPLETED:
                    result_data = BacktestResultCreate(
                        ...
                        metrics={
                            "total_return": round(0.05 + (hash(backtest.id) % 100) / 1000, 4),
                            "sharpe_ratio": round(1.2 + (hash(backtest.id) % 50) / 100, 2),
                            "max_drawdown": round(-0.08 - (hash(backtest.id) % 30) / 1000, 4),
                            "win_rate": round(0.55 + (hash(backtest.id) % 30) / 100, 3),
                            "profit_factor": round(1.3 + (hash(backtest.id) % 20) / 100, 2),
