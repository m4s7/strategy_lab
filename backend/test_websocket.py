...
async def test_websocket_connection():
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            print(f"   Received: {response_data['type']} - {response_data.get('data', {}).get('action', 'N/A')}")
            ...
            print(f"   Received pong: {pong_data['type']} (ID: {pong_data.get('id', 'N/A')})")
            ...
            print("        -d '{\"topic\": \"test:demo\", \"message\": {\"test\": true, \"value\": 42}}'")
            ...
            try:
                for i in range(5):
                    ...
                    print(f"   📨 Received: {msg_data['type']}:{msg_data.get('topic', 'N/A')} - {msg_data.get('data', {})}")
            except asyncio.TimeoutError:
                print("   No messages received (this is normal if no broadcasts were sent)")
...
async def test_websocket_stats():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            # Test stats endpoint
            async with session.get('http://localhost:8000/api/v1/websocket/stats') as response:
            ...
            async with session.get('http://localhost:8000/api/v1/websocket/topics') as response:
                if response.status == 200:
                    data = await response.json()
