...
def generate_mock_equity_curve(start_date: str, end_date: str, initial_capital: float) -> List[EquityPoint]:
...
def calculate_mock_metrics(equity_curve: List[EquityPoint], initial_capital: float) -> PerformanceMetrics:
    """Calculate performance metrics from equity curve."""
    if not equity_curve:
        return PerformanceMetrics(
            total_return=0, total_return_pct=0, sharpe_ratio=0, sortino_ratio=0,
            calmar_ratio=0, max_drawdown=0, max_drawdown_pct=0, volatility=0,
            var_95=0, cvar_95=0
        )
    ...
    if daily_returns:
        ...
        volatility = math.sqrt(sum((r - mean_return)**2 for r in daily_returns) / len(daily_returns))
        sharpe_ratio = (mean_return * 252) / (volatility * math.sqrt(252)) if volatility > 0 else 0
        
        # Sortino ratio (downside deviation)
        downside_returns = [r for r in daily_returns if r < 0]
        downside_dev = math.sqrt(sum(r**2 for r in downside_returns) / len(downside_returns)) if downside_returns else 0
        sortino_ratio = (mean_return * 252) / (downside_dev * math.sqrt(252)) if downside_dev > 0 else 0
    else:
        volatility = 0
        sharpe_ratio = 0
        sortino_ratio = 0
    ...
    if daily_returns:
        ...
        var_95 = abs(sorted_returns[var_index]) * initial_capital if var_index < len(sorted_returns) else 0
        
        # CVaR (average of worst 5%)
        worst_returns = sorted_returns[:var_index+1]
        cvar_95 = abs(sum(worst_returns) / len(worst_returns)) * initial_capital if worst_returns else 0
    else:
        var_95 = 0
        cvar_95 = 0
