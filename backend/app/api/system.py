...
async def get_system_stats(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    completed_today = sum(row.count for row in results if row.status == BacktestStatus.COMPLETED.value)
    failed_today = sum(row.count for row in results if row.status == BacktestStatus.FAILED.value)
