    ...
    async def simulate_execution(execution_id: str):
        ...
        try:
            ...
            for i in range(20, 90):
                ...
                execution.progress.current_tick = int(execution.progress.total_ticks * i / 100)
                ...
                if random.random() > 0.7:  # 30% chance of a trade
                    ...
                    execution.metrics.win_rate = (
                        execution.metrics.winning_trades / execution.metrics.total_trades * 100
                        if execution.metrics.total_trades > 0 else 0
                    )
            ...
            execution.errors.append(ExecutionError(
...
async def control_execution(
    ...
    if request.action == "cancel":
        ...
        if execution.status == ExecutionStatus.PROCESSING:
            ...
            raise HTTPException(status_code=400, detail="Cannot pause execution in current state")
        ...
        if execution.status == ExecutionStatus.PAUSED:
            ...
            raise HTTPException(status_code=400, detail="Cannot resume execution in current state")
...
async def get_queue_status():
    ...
    queue_executions = [MOCK_EXECUTIONS[eid] for eid in EXECUTION_QUEUE if eid in MOCK_EXECUTIONS]
    
    return {
        "total_in_queue": len(EXECUTION_QUEUE),
        "running": len([e for e in queue_executions if e.status == ExecutionStatus.PROCESSING]),
        "queued": len([e for e in queue_executions if e.status == ExecutionStatus.QUEUED]),
        "paused": len([e for e in queue_executions if e.status == ExecutionStatus.PAUSED]),
