...
async def get_data_availability():
    ...
    for contract, info in MOCK_CONTRACTS.items():
        ...
        if random.random() > 0.8:  # 20% chance of having gaps
            gap_start = datetime.fromisoformat(info["start_date"]) + timedelta(days=random.randint(20, 60))
...
async def get_available_contracts():
    ...
    for i, (contract, info) in enumerate(MOCK_CONTRACTS.items()):
        ...
        is_front = i == len(MOCK_CONTRACTS) - 2  # Second to last is typically front month
...
async def validate_data_configuration(config: DataConfiguration):
    ...
    for contract in config.contracts:
        ...
        available_data.append(DataAvailability(
            contract=contract,
    ...
    if len(config.contracts) > 5:
        warnings.append("Selecting more than 5 contracts may significantly increase processing time")
...
async def estimate_performance(config: DataConfiguration):
    ...
    if estimated_duration > 3600:
        warnings.append(f"Estimated processing time exceeds 1 hour ({estimated_duration // 3600}h {(estimated_duration % 3600) // 60}m)")
    
    if estimated_memory > 8000:
        warnings.append(f"Estimated memory usage ({estimated_memory}MB) may require a high-memory system")
    ...
    if len(config.contracts) > 3:
        recommendations.append("Consider processing contracts separately for better performance")
    ...
    for i in range(limit):
        ...
        if data_level == "L1":
            ...
            sample = {
                ...
                "volume": random.randint(1, 10),
                "bid_depth": [
                    {"price": base_price + price_change - 0.25 - i*0.25, "size": random.randint(5, 20)}
                    for i in range(5)
                ],
                "ask_depth": [
                    {"price": base_price + price_change + 0.25 + i*0.25, "size": random.randint(5, 20)}
