...
class TestOrderBookAnalytics:
    ...
    def test_calculate_depth_weighted_price(self):
        ...
        expected = (Decimal('100.00') * 1000 + Decimal('99.75') * 500 + Decimal('99.50') * 250) / 1750
...
class TestOrderBook:
    ...
    def test_multiple_bid_levels(self):
        ...
        assert bid_prices == [Decimal('100.25'), Decimal('100.0'), Decimal('99.75'), Decimal('99.50')]
    ...
    def test_multiple_ask_levels(self):
        ...
        assert ask_prices == [Decimal('100.25'), Decimal('100.50'), Decimal('100.75'), Decimal('101.0')]
    ...
    def test_process_dataframe(self):
        ...
        df = pd.DataFrame({
            'timestamp': [1609459200000000000, 1609459200000000001, 1609459200000000002],
    ...
    def test_book_validation(self):
        ...
        book._bid_levels[Decimal('100.50')] = OrderBookLevel(price=Decimal('100.50'), volume=1000)
...
class TestOrderBookReconstructor:
    ...
    def test_reconstruct_from_file(self):
        ...
        test_data = pd.DataFrame({
            'level': ['2', '2', '2', '2'],
            'timestamp': [1609459200000000000, 1609459200000000001, 1609459200000000002, 1609459200000000003],
            'mdt': [OrderBook.MDT_BID, OrderBook.MDT_ASK, OrderBook.MDT_BID, OrderBook.MDT_ASK],
            'price': [100.0, 100.25, 99.75, 100.50],
            'volume': [1000, 500, 750, 250],
            'operation': [OrderBook.OP_ADD, OrderBook.OP_ADD, OrderBook.OP_ADD, OrderBook.OP_ADD]
        })
    ...
    def test_reconstruct_with_timestamp_filter(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet', delete=False) as tmp_file:
            ...
            snapshot = reconstructor.reconstruct_from_file(tmp_file.name, target_timestamp)
    ...
    def test_reconstruct_time_series(self):
        ...
        test_data = pd.DataFrame({
            ...
            'mdt': [OrderBook.MDT_BID, OrderBook.MDT_ASK, OrderBook.MDT_BID, OrderBook.MDT_ASK, OrderBook.MDT_BID, OrderBook.MDT_ASK],
...
class TestIntegration:
    ...
    def test_market_stress_scenario(self):
        ...
        for i in range(10):
            ...
            book.process_operation(timestamp + i, OrderBook.MDT_BID, bid_price, 1000, OrderBook.OP_ADD)
            
            # Asks from 100.25 up to 101.50 (0.25 increment)  
            ask_price = 100.25 + i * 0.05
            book.process_operation(timestamp + i + 10, OrderBook.MDT_ASK, ask_price, 500, OrderBook.OP_ADD)
        ...
        for i in range(3):
            price = 100.00 - i * 0.05
            book.process_operation(timestamp + 100 + i, OrderBook.MDT_BID, price, 0, OrderBook.OP_REMOVE)
    ...
    def test_performance_counters(self):
        ...
        book.process_operation(1609459200000000000, -1, 100.0, 1000, OrderBook.OP_ADD)  # Invalid MDT
        book.process_operation(1609459200000000000, OrderBook.MDT_BID, -100.0, 1000, OrderBook.OP_ADD)  # Invalid price
